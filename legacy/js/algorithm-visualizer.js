...
var Algorithm = (function() {
  ...
  var ALGO_INFO = {
    ...
    insertion: { name:'插入排序', time:'O(n²)', space:'O(1)', stable:true,
    ...
    merge: { name:'归并排序', time:'O(nlogn)', space:'O(n)', stable:true,
      ...
      pseudo: ['function mergeSort(lo, hi):',
               '  if lo >= hi: return',
               '  mid = (lo + hi) / 2',
               '  mergeSort(lo, mid)',
               '  mergeSort(mid+1, hi)',
    ...
    counting: { name:'计数排序', time:'O(n+k)', space:'O(k)', stable:true,
      ...
      pseudo: ['max = max(arr)',
               ...
               '    arr[idx++] = i',
               '    count[i]--'] },
    binary: { name:'二分查找', time:'O(logn)', space:'O(1)', stable:true,
      desc:'有序数组中，每次比较中间元素，缩小一半范围。类似猜数字游戏。',
      pseudo: ['target = arr[mid] // 目标值',
               'lo = 0, hi = n-1',
               'while lo <= hi:',
               '  mid = (lo + hi) / 2',
               '  if arr[mid] == target:',
  ...
  function sleep() {
    ...
    return new Promise(function(resolve) {
      ...
      if (stepMode) {
        // In step mode, wait for user to click step button
        stepResolve = resolve;
        var statusEl = document.getElementById('algo-status');
        if (statusEl) { statusEl.textContent = '⏸ 单步模式 — 点击单步继续'; statusEl.className = 'algo-status paused'; }
      } else if (paused) {
        // Wait until unpaused
        var checkPaused = setInterval(function() {
          if (!paused) { clearInterval(checkPaused); resolve(); }
          if (stopFlag) { clearInterval(checkPaused); resolve(); }
        }, 50);
      } else {
  ...
  function updateInfo() {
    ...
    if (desc) desc.innerHTML = '📊 <strong>' + info.name + '</strong> — ' + info.desc +
      (info.stable ? ' <span class="stable-badge">稳定排序</span>' : ' <span class="unstable-badge">不稳定</span>');
    // Render pseudocode
    renderPseudo(info);
  }

  function renderPseudo(info) {
    var el = document.getElementById('algo-pseudo');
    if (!el || !info.pseudo) { if (el) el.textContent = ''; return; }
    var html = info.pseudo.map(function(line, i) {
      var cls = 'pseudo-line';
      // Highlight lines based on current algorithm phase
      return '<span class="' + cls + '" data-line="' + i + '">' + escHtml(line) + '</span>';
    }).join('\n');
    el.innerHTML = html;
  }

  function highlightPseudoLine(lineIdx) {
    var lines = document.querySelectorAll('#algo-pseudo .pseudo-line');
    lines.forEach(function(l) { l.classList.remove('line-active'); });
    if (lineIdx >= 0 && lines[lineIdx]) {
      lines[lineIdx].classList.add('line-active');
    }
  }
  ...
  async function start() {
    ...
    if (currentAlgo !== 'binarysearch' && currentAlgo !== 'binary') resetArr();
  ...
  function init() {
    ...
    // 单步执行
    document.getElementById('btn-algo-step').addEventListener('click', function() {
      if (!running) {
        // First step: start in step mode
        stepMode = true;
        start();
      } else {
        // Advance one step
